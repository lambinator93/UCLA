...
for n in range(0, N):
	for m in range (0, M):
		H[n,m] = (n+(1/2))*kron(n,m) -(9/2)*((n+(1/2))*kron(n,m) + (1/2)*ma.sqrt((n+1)*(n+2))*kron(n,m-2) + (1/2)*ma.sqrt((n-1)*n)*kron(n,m+2)) + (((1/4)*(6*n*n + 6*n + 3))*kron(n,m)
...
np.savetxt("double_well_wavefunctions.dat", np.c_[S])
np.savetxt("double_well_energies.dat",np.c_[evals])